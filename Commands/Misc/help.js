  ...
  async execute(interaction, client) {
    ...
    if (!helpcmd) {
      ...
      const embed = new MessageEmbed()
        ...
        .setURL('https://damianji.herokuapp.com/commands')
        ...
        const select = async (interaction) => {
          ...
          readdirSync("./Commands/").forEach((dir) => {
            ...
            const cmds = commands.map((command) => {
              ...
              let check = client.commands.get(name).context
              if (check) return;
              if (!usg) {
      ...
      if (cots.includes(helpcmd.toLowerCase())) {
        const combed = new MessageEmbed()
          .setTitle(
            `__${helpcmd.charAt(0).toUpperCase() + helpcmd.slice(1)
            } Commands__`
          )
          .setDescription(
            `Use \`/help\` followed by a command name to get more information on a command.\nFor example: \`/help botinfo\`.\n\n`
