...
module.exports = {
  ...
  usage: "/reactrole <role1> [role2] [role3]... [role5] [embed message]",
  options: [
    {
      name: "role1",
  ...
  async execute(interaction, guild, client) {
    ...
    const Response = new MessageEmbed()
      .setColor('AQUA')
    if (embdmsg) await Response.setDescription(`${embdmsg.replaceAll('\\n', `\n`)}`)
    ...
    if (role5) {
      if (role5.position > interaction.member.roles.highest.position) {
        return interaction.reply({ embeds: [errembd], ephemeral: true });
      } else if (role5.position > interaction.guild.me.roles.highest.position) return interaction.reply({ embeds: [errbot], ephemeral: true });
    }
