...
module.exports = {
	...
	async execute(interaction) {
		...
		if (
			!member.roles.cache.find((r) => r.id === TicketSetup.Handlers) &&
		...
		DB.findOne({ ChannelID: channel.id }, async (err, docs) => {
			...
			if (!docs)
				return interaction.reply({
					content:
						'You cannot create another Ticket, please close the current one or use the same ticket.',
					ephemeral: true,
				});
			...
			switch (customId) {
				...
				case 'oen':
					...
					const GD = new MessageEmbed().setDescription(
						`This Ticket Is Now Opened By ${interaction.user}`
					);
